...
class OrderController {
  ...
  async store({ request, response, auth, transform }) {
    ...
    try {
      const items = request.input('items') // return array
      const client = await auth.getUser()
      ...
      // emite um broacast no websocket
      ...
      return response.status(400).send({
        message: 'Não foi possível fazer seu pedido nesse momento!',
      })
    }
  }
  ...
  async update({ params, request, response, auth, transform }) {
    ...
    try {
      ...
      await trx.rollback()
      return response.status(400).send({
        message: 'Não foi possível atulizar seu pedido nesse momento!',
      })
    }
  }
}
